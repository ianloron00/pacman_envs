...
class Extractor:
    name_features = np.array(["bias", "#-of-scared-ghosts-1-step-away", "#-of-scared-ghosts-2-steps-away",
                    ...
                    "has-not-a-wall-west", "eats-capsule", "pacman-moved"])
    ...
    def getFeatures(self,state,action):
        ...
        features["eats-food"] = 1.0 if (not features["#-of-active-ghosts-1-step-away"] and food[next_x][next_y]) else -1.0
        features["run-to-catch-closest-fruit"]=float(int(action==fruit.dir)/(fruit.dist+1)) if fruit != None else -1.0
        ...
        features["pacman-moved"] = 1.0 if (next_x, next_y) != (x, y) else -1.0  
